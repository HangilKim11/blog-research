...
> 限度額を上げると貸し倒れは増えるのか? データは逆(限度額↑ → 貸し倒れ↓)を示します。この逆説を逐次残差化(DML型)デバイアシングで扱い、公開データ3種で検証します。
>
...
生データの「限度額が高いほど貸し倒れ率が低い」という逆説は、**選択バイアス**(信用の良い人に高い限度額が付与される)によるものです。このリポジトリはそのバイアスを除く手法を実装し、**逆説が実際にいつ反転するのか**を3つのデータで解明します。

解説記事(韓国語): **[han-co.com/ko/blog/credit-limit-debiasing](https://han-co.com/ko/blog/credit-limit-debiasing)**

## 主な結果

| 与信の種類 | raw 限度額-貸し倒れ | デバイアシング後 | 事例 |
|---|---|---|---|
| 未使用のリボ枠 | 負(逆説) | ≈ 0 | UCI · Lending Club · Home Credit カード |
| 引き出された与信 + 弱い選択バイアス | 正 | 正 | Lending Club ローン額 |
| **引き出された与信 + 強い選択バイアス** | **負(逆説)** | **正(反転)** | **Home Credit 本ローン** |

逆説がデバイアシングで反転するには ① 引き出された与信(実負担)と ② 強い選択バイアスが同時に揃う必要があります。(前提0: 貸し倒れ定義が本物の信用損失を捉えていること。Home Credit カードの `SK_DPD≥90` は「放置された少額残高」を捉え、符号が壊れる落とし穴。)

**結論:** 手法(デバイアシング)は妥当で移植可能ですが、「限度額↑→貸し倒れ↑」は普遍法則ではありません。実務ポートフォリオで ① パススルー `dBalance/dLimit` と ② 貸し倒れ定義を直接点検する必要があります。
...
K-fold 交差適合(cross-fitting)残差化 + isotonic キャリブレーション + 残差加重 + 線形2次ステージ(DML)。限度額・残高・貸し倒れを信用度の特徴量から残差化して 限度額→残高→貸し倒れ の経路を分離し、反事実(counterfactual)で限度額変化の効果を推定します。弱い残差シグナルには GBM ではなく線形モデルを使い、過学習を避けます。
